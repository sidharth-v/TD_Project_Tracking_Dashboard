...
BASE_COLUMNS = [
    "S_No", "Unknown_1", "Job_Ref", "LPO_Ref", "Customer", "Project_Name",
    "Region", "Location", "Unknown_8", "Equi_Design", "Technical_Submittal",
    "Drawing", "ELS", "BOM", "Out_Door", "Indoor", "CR_Panels",
    "CR_Ins_Materials", "Doors", "Ref_Inst_Materials", "CCP", "Display_CCP",
    "Floor_Heater", "Cabinets", "Any_Special", "Work_Status", "Unknown_26",
    "Material_Status", "Overall_Progress", "Priority", "Status",
    "Engineering_Pct", "Delivery_Pct", "Execution_Pct",
]

ENGINEERING_ITEMS = ["Equi_Design", "Technical_Submittal", "Drawing", "ELS", "BOM"]

DELIVERY_ITEMS = [
    "Out_Door", "Indoor", "CR_Panels", "CR_Ins_Materials", "Doors",
    "Ref_Inst_Materials", "CCP", "Display_CCP", "Floor_Heater",
    "Cabinets", "Any_Special",
]
...
def clean_status(value):
    ...
    return value if value in allowed else "Not Started"
...
def clean_material(value):
    ...
    return value if value in allowed else "Not Ordered"
...
def convert_onedrive_link(url):
    """
    Converts personal OneDrive share links to direct content links.
    Use a share link like: https://1drv.ms/x/s!...
    """
    if not url:
        return url
...
def download_excel_from_url(url):
    ...
    response = requests.get(final_url, timeout=45, allow_redirects=True)
    response.raise_for_status()

    content_type = response.headers.get("Content-Type", "").lower()
    content_start = response.content[:300].lower()

    if b"<html" in content_start or b"<!doctype html" in content_start or "text/html" in content_type:
        raise RuntimeError(
            "The OneDrive link is opening a web page, not downloading the Excel file. "
            "Use a personal OneDrive share link like https://1drv.ms/x/s!... with 'Anyone with the link can view'."
        )
...
def parse_excel(file_bytes):
    raw = pd.read_excel(BytesIO(file_bytes), sheet_name="Project_Master", header=None, engine="openpyxl")
    ...
    text_cols = ["Job_Ref", "LPO_Ref", "Customer", "Project_Name", "Region", "Location", "Work_Status"]
...
def make_top_bar(df, column, top_n=10, height=340, color="#2563eb"):
    ...
    fig = px.bar(counts, x="Count", y=column, orientation="h", text="Count")
...
def make_delivery_items_chart(df):
    ...
    for col in DELIVERY_ITEMS:
        ...
        rows.append({
            ...
            "Part Done": (df[col] == "PART.DONE").sum(),
            "Not Done": (df[col] == "N.DONE").sum(),
        })

    chart_df = pd.DataFrame(rows)

    fig = go.Figure()
    fig.add_trace(go.Bar(y=chart_df["Item"], x=chart_df["Done"], orientation="h", name="Done",
                         marker_color="#16a34a", text=chart_df["Done"], textposition="outside", cliponaxis=False))
    fig.add_trace(go.Bar(y=chart_df["Item"], x=chart_df["Part Done"], orientation="h", name="Part Done",
                         marker_color="#eab308", text=chart_df["Part Done"], textposition="outside", cliponaxis=False))
    fig.add_trace(go.Bar(y=chart_df["Item"], x=chart_df["Not Done"], orientation="h", name="Not Done",
                         marker_color="#dc2626", text=chart_df["Not Done"], textposition="outside", cliponaxis=False))
...
def make_progress_bucket_chart(df):
    ...
    bucket = pd.cut(df["Overall_Progress"], bins=bins, labels=labels, include_lowest=True)
    counts = bucket.value_counts().reindex(labels).fillna(0).astype(int).reset_index()
    counts.columns = ["Progress Bucket", "Count"]

    fig = px.bar(counts, x="Count", y="Progress Bucket", orientation="h", text="Count")
...
def make_phase_by_project_chart(df):
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(y=project_labels, x=temp["Engineering_Pct"], name="Engineering",
                         orientation="h", marker_color="#2563eb"))
    fig.add_trace(go.Bar(y=project_labels, x=temp["Delivery_Pct"], name="Delivery",
                         orientation="h", marker_color="#7c3aed"))
    fig.add_trace(go.Bar(y=project_labels, x=temp["Execution_Pct"], name="Execution",
    ...
    fig.update_layout(
        ...
        xaxis=dict(title=None, range=[0, 105], ticksuffix="%", showgrid=True, gridcolor="#e5e7eb", zeroline=False),
        yaxis=dict(title=None, autorange="reversed"),
    )
    return fig
...
quick_view = st.sidebar.radio("Quick Views", ["All", "Completed", "In Progress", "On Hold", "Not Started"])
...
with c1:
    fig = make_count_bar(filtered_df, "Status", order=["Completed", "In Progress", "On Hold", "Not Started", "Cancelled"])
    render_chart_card("Project Status", fig)

with c2:
    fig = make_count_bar(filtered_df, "Material_Status", order=["Delivered", "Partially Delivered", "Ordered", "Not Ordered"])
    render_chart_card("Material Status", fig)

with c3:
    fig = make_count_bar(filtered_df, "Priority", order=["High", "Medium", "Low"])
...
render_chart_card("Phase Progress by Project", fig, "Engineering / Delivery / Execution — sorted by Overall %")
...
st.dataframe(table_df, use_container_width=True, hide_index=True, height=620)
